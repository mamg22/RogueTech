...
def no_session_handler(e):
    flash(
        "No ha iniciado sesión, por favor inicie sesión e intentelo de nuevo", "warning"
    )
    return redirect("/")
...
def last_admin_remove_handler(e):
    flash(
        "Este usuario es el unico administrador del sistema, no sera posible cambiar su rol hasta que asigne otro administrador",
        "error",
    )
...
def not_found_handler(e):
    return render_template(
        "error.html", message="No se ha podido encontrar esta página", details=e
    ), 404
...
def puntuaciones():
    ...
    return render_template(
        ...
        page=page,
        SCORES_PER_PAGE=model.SCORES_PER_PAGE,
    )
...
def usuarios():
    ...
    return render_template(
        ...
        page=page,
        USERS_PER_PAGE=model.USERS_PER_PAGE,
    )
...
def usuario():
    ...
    try:
        ...
        logged_in = False
    return render_template(
        "usuario.html", user=userdata, is_following=is_following, logged_in=logged_in
    )
...
def login():
    ...
    if request.method == "POST":
        ...
        if user := model.login(nombre, clave):
            ...
            flash(
                "Nombre de usuario o constraseña incorrecto, por favor intentelo de nuevo",
                "error",
            )
...
def new_account():
    if request.method == "POST":
        ...
        if model.create_account(nombre, clave, pin):
            ...
            flash(
                "Ya existe un usuario registrado con este nombre, por favor, elija otro nombre",
                "error",
            )
...
def follow():
    ...
    return render_template(
        "fragments/followinfo.html", user=userdata, is_following=is_following
    )
